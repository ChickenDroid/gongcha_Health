...
public class batsal_Adapter extends RecyclerView.Adapter<batsal_Adapter.ItemViewHolder> {
    /*  리사이클러뷰를 사용하는경우가 많이 있지만 뱃살 탭에서는 하나만 사용하므로 따로 어댑터를 안꼬이게 더만들음  */
    private ArrayList<Traing_data> listData = new ArrayList<>();
    @NonNull
    @Override
    public batsal_Adapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.training1_listitem, parent, false);
        return new batsal_Adapter.ItemViewHolder(view);
    }


    @Override
    public void onBindViewHolder(@NonNull batsal_Adapter.ItemViewHolder holder, final int position) {
